...
const ManuSpeakerCard = () => {
  ...
  const mintSpeakerNft = async () => {
    ...
    if (walletAddress.trim().length !== 42) {
      ...
      address = await web3.eth.ens.getAddress(walletAddress.trim());
    ...
    if (address) {
      try {
        const url = `${config.apiBaseUrl}/mintEthCcSpeakerNft`;
...
export default ManuSpeakerCard;
